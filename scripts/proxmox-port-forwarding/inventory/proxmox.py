...
# Enter all the filters you want to apply, these are tags applied to each VM on creation by Terraform
project_filters = {
    "filters": [
    {
        "Team": "example",
...
def CreateIPTablesScript(node_name, node_nic_name, vmbr0_cidr):
    file_start = open("port-forwarding-script-" + node_name + ".sh", "w")
    basic_script = ["#!/bin/bash \n",
                    "read -p \"ARE YOU SURE YOU MANUALLY REVIEWED THE SCRIPT????\" yn \n",
                    "case $yn in\n",
                    "   [Nn]* ) exit 1;;\n",
                    "    * ) echo \"Please answer y or n.\";; \n",
                    "esac\n",
                    "echo 1 > /proc/sys/net/ipv4/ip_forward \n",
                    "\n ############ Flushing old configs ################ \n",
                    "iptables -F \n",
                    "iptables -F -t nat \n",
                    "iptables -X \n",
                    "iptables -X -t nat \n",
                    "iptables -Z \n",
                    "iptables -Z -t nat \n",
                    "\n ############ Accept all connections, Clean up in the future! ################ \n",
                    "iptables -A OUTPUT -j ACCEPT \n",
                    "iptables -A FORWARD -j ACCEPT \n",
                    "\n ############ Writing basic NAT rules for internet access ################ \n",
                    "iptables -t nat -A POSTROUTING -s '" + vmbr0_cidr + "' -o vmbr0 -j MASQUERADE \n",
                    "iptables -t nat -A POSTROUTING -s '" + vmbr0_cidr + "' -o " + node_nic_name + " -j MASQUERADE \n",
                    ]
    file_start.writelines(basic_script)
    file_start.close()


def CloseIPTablesScript(node_name):
    file1 = open("port-forwarding-script-" + node_name + ".sh", "a")
    file1.write("\n echo \"DONE!\" \n")
    file1.close()

def AppendIPTablesScript(node_name, ip_address, port):
    # Append all the port rules for the individual hosts
    file1 = open("port-forwarding-script-" + node_name + ".sh", "a")
    file1.write("\n ############ Writing Port forward rules for IP: " + ip_address + ":" + port + " ################ \n")
    file1.write("iptables -t nat -A PREROUTING -p tcp --dport " +  port + " -j DNAT --to " + ip_address + ":" + port + "\n")
    file1.write("iptables -t nat -A PREROUTING -p udp --dport " +  port + " -j DNAT --to " + ip_address + ":" + port + "\n")
    file1.write("\n")
    file1.close()

def FetchPublicIPAddress(node):
    if node == "<PROXMOX-NODE-NAME>":
        return '<IP-ADDRESS>'


def FilterChecker(description):
    description_json = json.loads(description)
    for filter in project_filters["filters"]:
            if filter["Project"] == description_json["Project"] and filter["SubProject"] == description_json["SubProject"]:
                return True
    return False
...
class ProxmoxAPI(object):
    ...
    def node_nic_names(self, node):
        try:
            return self.get('api2/json/nodes/{0}/network'.format(node))
        except:
            return False
...
def main_list(options, config_path):
    ...
    for node in proxmox_api.nodes().get_names():
        ...
        node_variables = qemu_list.get_variables()


        node_nic_names = proxmox_api.node_nic_names(node)

        node_nic_name = ""
        vmbr0_cidr = ""
        for nic in node_nic_names:
            if nic['method'] == "static" and nic['type'] == "eth":
                node_nic_name = nic['iface']
            if nic['iface'] == "vmbr0":
                vmbr0_cidr = nic['cidr']

        CreateIPTablesScript(node, node_nic_name, vmbr0_cidr)

        for vm in node_variables:
            if node_variables[vm]['proxmox_template'] == "":
                try:
                    type = results['_meta']['hostvars'][vm]['proxmox_type']
                except KeyError:
                    type = 'qemu'
                description = proxmox_api.vm_description_by_type(node, node_variables[vm]['proxmox_vmid'], type)['description']
                # description = json.loads(description)
                if FilterChecker(description):
                    results['all']['hosts'].append(node_variables[vm]['proxmox_name'])
                    results['_meta']['hostvars'].update({vm:node_variables[vm]})
                    ...
                    if node_ip:
                        for vm_interface in node_ip['result']:
                            if vm_interface['name'] == "eth0" or vm_interface['name'] == "ens18":
                                results['_meta']['hostvars'][vm]['ansible_host'] = vm_interface['ip-addresses'][0]['ip-address']
                                results['_meta']['hostvars'][vm]['display_emoji'] = node_variables[vm]['proxmox_name']
                                results['_meta']['hostvars'][vm]['beacon_udp_port'] = beacon_port_start + int(vmid)
                                results['_meta']['hostvars'][vm]['beacon_tcp_port'] = beacon_port_start + int(vmid)
                                results['_meta']['hostvars'][vm]['ansible_ssh_common_args'] = '-C  -o StrictHostKeyChecking=no -o ProxyCommand="ssh -W %h:%p devops@' + str(FetchPublicIPAddress(node)) + '"'
                                AppendIPTablesScript(node, vm_interface['ip-addresses'][0]['ip-address'], str(beacon_port_start + int(vmid)))
                    try:
                    ...
                    if status == 'running':
                        if 'beacon_node' not in results:
                            results['beacon_node'] = {
                                'hosts': []
                            }
                        results['beacon_node']['hosts'] += [vm]
