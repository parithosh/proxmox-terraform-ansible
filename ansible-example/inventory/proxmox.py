...
def FilterChecker(description):
    ...
    for attribute in project_filters["filters"]:
        if project_filters["filters"][attribute] != description_json[attribute]:
            return False
    return True
                    ...
                    if node_ip:
                        for vm_interface in node_ip['result']:
                            if vm_interface['name'] == "eth0" or vm_interface['name'] == "ens18":
                                ...
                                results['_meta']['hostvars'][vm]['public_ip_address'] = FetchPublicIPAddress(node)
                    try:
